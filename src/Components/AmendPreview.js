    ...
    // })
        ...
        setVPAN(resvendor?.data?.msg?.filter(e=>e.sl_no==res?.data?.msg?.vendor_id)[0].vendor_pan)
          axios.post(url+'/api/getpoitem',{id:id}).then(resItem=>{
            ...
            axios.post(url+'/api/getpoterms',{id:id}).then(resTerm=>{
              console.log(resTerm)
          ...
          axios.post(url+'/api/getpopayterms',{id:id}).then(resPay=>{
            console.log(resPay)
            ...
            axios.post(url+'/api/getpodelivery',{id:id}).then(resDel=>{
              console.log(resDel)
            ...
            //   localStorage.setItem('notes',resDel?.data?.msg[0]?.po_notes)
              axios.post(url+'/api/getpomore',{id:id}).then(resMore=>{
                console.log(resMore)
                ...
                axios.post(url+'/api/getpocomments',{id:id}).then(resCom=>{
                  // setTimeline([])
                  console.log(resCom)
...
<Skeleton width="40rem" className="mb-2 w-full bg-gray-300"></Skeleton>
<Skeleton width="40rem" className="mb-2 w-full bg-gray-300"></Skeleton>
<Skeleton width="40rem" className="mb-2 w-full bg-gray-300"></Skeleton>
<Skeleton width="40rem" className="mb-2 w-full bg-gray-300"></Skeleton>
<Skeleton width="40rem" className="mb-2 w-full bg-gray-300" height="4rem"></Skeleton>
...
<div className="relative overflow-x-auto">
    <table className="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-400">
      ...
      <div className="my-2 w-full p-2 text-black font-semibold  border-2 border-blue-400 bg-blue-400 rounded-lg">
       ...
       { termList.length>0 && termList.map(item=> <li>
    ...
    <table className="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-400">
        ...
        <tbody>
            <tr className="bg-white border-b text-nowrap dark:bg-gray-800 dark:border-gray-700">
                ...
                {price_basis_flag=='F'?'FOR':'EX-WORKS'}, {price_basis_desc}
                </td>
            </tr>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
