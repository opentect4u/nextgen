...
# );
    ...
    select = f"DISTINCT i.item_id, p.prod_name, i.quantity, d.rc_qty,d.mrn_no,d.invoice,pb.pur_req,pd.proj_name"
    
    schema = f"td_po_items i JOIN td_po_basic pb ON i.po_sl_no = pb.sl_no AND {criteria} LEFT JOIN td_item_delivery_details d ON i.item_id = d.prod_id AND d.po_no = pb.po_no LEFT JOIN md_product p on i.item_id=p.sl_no LEFT JOIN td_project pd on pb.project_id=pd.sl_no" if id.type=='W' else f"td_po_items i JOIN td_po_basic pb ON i.po_sl_no = pb.sl_no AND {criteria} LEFT JOIN td_item_delivery_details d ON i.item_id = d.prod_id AND d.po_no = pb.po_no LEFT JOIN md_product p on i.item_id=p.sl_no"
