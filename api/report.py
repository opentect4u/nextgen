...
async def getprojectpoc(id:MatVal):
    ...
    select = f"b.po_no,b.sl_no,i.item_id,i.quantity,i.discount,i.currency,i.discount_percent,d.mrn_no,m.invoice,DATE_FORMAT(m.invoice_dt,'%d/%m/%Y') as invoice_dt,i.cgst_id,i.sgst_id,i.igst_id,i.item_rt,(i.item_rt-i.discount)*i.quantity as net_unit_price,d.mrn_no,d.prod_id,d.rc_qty,st.qty, (select sum(qty) from td_stock_new where proj_id={id.proj_id} and item_id=st.item_id and in_out_flag=-1) stock_out_qty,concat(pd.prod_name,'(Part No.:',pd.part_no,', Article No.:',pd.article_no,', Model No.:',pd.model_no,', Make:',pd.prod_make,', Description:',pd.prod_desc,')') as prod_name, SUM(d.rc_qty) AS total_rc_qty, SUM(st.qty) AS total_qty"
