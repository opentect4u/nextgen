...
async def getprojectpoc(id:mrnpur):
    ...
    select = f" i.item_id, i.quantity,p.approved_ord_qty,SUM(d.rc_qty) AS rc_qty,GROUP_CONCAT(DISTINCT d.mrn_no) AS mrn_no,GROUP_CONCAT(DISTINCT d.invoice) AS invoice,DATE_FORMAT(inv.invoice_date,'%d/%m/%Y'), concat(pr.prod_name,'(Make:',pr.prod_make,', Part No.:',pr.part_no,',  Article No.:',pr.article_no,', Model No.:',pr.model_no,', Description:',pr.prod_desc,')') as prod_name"
