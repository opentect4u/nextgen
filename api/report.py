...
async def getprojectpoc(id:mrnpur):
    select = f"@a:=@a+1 '#', b.po_no 'PO No.',DATE_FORMAT(b.po_issue_date,'%d/%m/%Y') 'Date',IF(b.po_status='A','Approved',IF(b.po_status='U','Approval Pending',IF(b.po_status='D','Delivered','Partial Delivery'))) 'Status',COALESCE(p.proj_name,'Warehouse') 'Intended For',v.vendor_name 'Vendor',b.created_by 'Created By', ROUND(SUM(i.quantity * (i.item_rt - i.discount) * ((COALESCE(i.cgst_id, 0) + COALESCE(i.sgst_id, 0) + COALESCE(i.igst_id, 0)) / 100)), 2) AS 'GST', ROUND(SUM(i.quantity * (i.item_rt - i.discount)), 2) 'Basic Value', ROUND(SUM(i.quantity * (i.item_rt - i.discount) + i.quantity * (i.item_rt - i.discount) * ((COALESCE(i.cgst_id, 0) + COALESCE(i.sgst_id, 0) + COALESCE(i.igst_id, 0))) / 100), 2) AS 'Gross Value'"
