...
async def getprojectpoc(id:mrnpur):
    select = f" @a:=@a+1 AS serial_number,DATE_FORMAT(req.pur_date,'%d/%m/%Y') as pur_date,req.created_by,req.created_at, concat(pr.proj_name,'(',pr.proj_id,')') as proj_name, req.pur_no, sum(pur.qty) qty, sum(pur.ordered_qty) ordered_qty,sum(pur.approved_ord_qty) as approved_ord_qty, IF(sum(pur.approved_ord_qty)=0,'Not Ordered','Partially Ordered') as status"
