...
async def getprojectpoc(id:mrnpur):
    ...
    where = f"i.po_sl_no IN (SELECT sl_no FROM td_po_basic WHERE pur_req LIKE '%PR-1747891204%')  GROUP BY i.item_id) AS t1 LEFT JOIN td_purchase_items p ON t1.item_id = p.item_id AND p.pur_no LIKE '%PR-1747891204%' LEFT JOIN md_product pr ON pr.sl_no = t1.item_id LEFT JOIN td_item_delivery_details d ON t1.item_id = d.prod_id AND d.po_no IN (SELECT po_no FROM td_po_basic WHERE pur_req LIKE '%PR-1747891204%') LEFT JOIN  td_item_delivery_invoice inv ON inv.mrn_no = d.mrn_no, (SELECT @a:= 0) AS a"
    order = ""
    flag = 1 
    result = await db_select(select, schema, where, order, flag)
    return result
