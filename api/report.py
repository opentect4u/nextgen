...
async def getprojectpoc(id:stockoutreport):
    ...
    select = f"st.date,st.item_id,st.qty,st.created_by,concat(p.prod_name,'(Part No.:'p.part_no,', Article No.:',p.article_no,', Model No.:',p.model_no,', Make:',p.make,', Description:',p.prod_desc,')',) as prod_name,st.in_out_flag,st.ref_no,st.proj_id,pr.proj_name,st.created_at" if id.type=='P' else f"st.date,st.item_id,st.qty,st.created_by,concat(p.prod_name,'(Part No.:'p.part_no,', Article No.:',p.article_no,', Model No.:',p.model_no,', Make:',p.make,', Description:',p.prod_desc,')',) as prod_name,st.in_out_flag,st.ref_no,0 as proj_id,'Warehouse' as proj_name,st.created_at"
