...
async def getprojectpoc(id:MatVal):
    ...
    where = f"st.proj_id={id.proj_id} group by st.item_id, p.prod_name, i.cgst_id, i.sgst_id, i.igst_id, i.item_rt, i.discount"
    schema = f"td_po_items i join td_stock_new st on i.item_id=st.item_id left join md_product p ON st.item_id = p.sl_no"
    order = ""
    flag =1 
    result = await db_select(select, schema, where, order, flag)
    return result
