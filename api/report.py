...
async def getprojectpoc(id:mrnprojreport):
    ...
    select =f"SELECT DISTINCT, i.item_id, p.prod_name, pb.po_no, i.quantity,pi.approved_ord_qty, d.rc_qty,d.mrn_no,d.invoice,pb.pur_req,v.vendor_name,COALESCE(pd.proj_name, 'Warehouse') AS proj_name"
    schema =   f"td_po_items i LEFT JOIN td_po_basic pb ON i.po_sl_no = pb.sl_no LEFT JOIN td_item_delivery_details d ON i.item_id = d.prod_id AND d.po_no = pb.po_no LEFT JOIN md_product p ON i.item_id = p.sl_no LEFT JOIN td_project pd ON pb.project_id = pd.sl_no LEFT JOIN d_vendor v ON pb.vendor_id = v.sl_no LEFT JOIN td_purchase_items pi ON i.item_id = pi.item_id" if id.type == 'P' else f"td_po_items i LEFT JOIN td_po_basic pb ON i.po_sl_no = pb.sl_no AND " + criteria + " LEFT JOIN td_item_delivery_details d ON i.item_id = d.prod_id AND d.po_no = pb.po_no LEFT JOIN md_product p ON i.item_id = p.sl_no LEFT JOIN md_vendor v ON pb.vendor_id = v.sl_no LEFT JOIN td_purchase_items pi ON i.item_id = pi.item_id"


    where = "1 = 1"  # No filtering on td_item_delivery_details to allow missing delivery entries
