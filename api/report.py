...
async def getprojectpoc(id:MatVal):
    ...
    select = f"@a:=@a+1 serial_number,b.po_no,b.sl_no,i.item_id,i.quantity,i.discount,i.currency,i.discount_percent,d.mrn_no,m.invoice,DATE_FORMAT(m.invoice_dt,'%d/%m/%Y') as invoice_dt,i.cgst_id,i.sgst_id,i.igst_id,i.item_rt,(i.item_rt-i.discount)*i.quantity as net_unit_price, if(i.igst_id>0,(i.item_rt-i.discount)*i.quantity*i.igst_id/100, ((i.item_rt - i.discount) * i.quantity * i.cgst_id / 100) + ((i.item_rt - i.discount) * i.quantity * i.sgst_id / 100)) as total_gst, IF(i.igst_id > 0,( i.item_rt - i.discount ) * i.quantity * i.igst_id / 100+((i.item_rt-i.discount)*i.quantity), ( ((i.item_rt - i.discount ) * i.quantity * i.cgst_id / 100)+((i.item_rt-i.discount)*i.quantity) ) + (( (i.item_rt - i.discount ) * i.quantity * i.sgst_id / 100 )+((i.item_rt-i.discount)*i.quantity))) AS total,d.mrn_no,d.prod_id,d.rc_qty,st.qty, (select qty from td_stock_new where proj_id={id.proj_id} and item_id=st.item_id and in_out_flag=-1 and ref_no like '%REQ%') stock_out_qty,concat(pd.prod_name,'(Part No.:',pd.part_no,', Article No.:',pd.article_no,', Model No.:',pd.model_no,', Make:',pd.prod_make,', Description:',pd.prod_desc,')') as prod_name, SUM(d.rc_qty) AS total_rc_qty, SUM(st.qty) AS total_qty"
...
async def getprojectpoc(id:mrnpur):
    select = f" @a:=@a+1 AS serial_number,pur.ordered_qty,pur.approved_ord_qty,(pur.ordered_qty - pur.approved_ord_qty) AS free_qty, pur.item_id,CONCAT(pd.prod_name,'(Part No.:', pd.part_no,', Article No.:', pd.article_no,', Model No.:', pd.model_no,', Make:', pd.prod_make,', Description:', pd.prod_desc, ')') AS prod_name,(select group_concat(po_no,if(po_status='P','(In Progress)',if(po_status='U','(Pending)','(Approved)'))) from td_po_basic where pur_req like '%{id.pur_no}%') as po_no"
    schema = f'''td_purchase_items pur left join md_product pd on pd.sl_no = pur.item_id left join td_po_basic po on FIND_IN_SET(po.po_no, pur.pur_no) > 0 cross join (SELECT @a := 0) AS a
    '''
