...
async def getprojectpoc(id:MatVal):
    ...
    select = f"@a:=@a+1 serial_number,GROUP_CONCAT(m.invoice) as invoice,GROUP_CONCAT(DATE_FORMAT(m.invoice_dt,'%d/%m/%Y')) as invoice_dt,i.cgst_id,i.sgst_id,i.igst_id,(i.item_rt-i.discount)*sum(d.rc_qty) as net_unit_price, IF(i.igst_id > 0,( i.item_rt - i.discount ) * sum(d.rc_qty) * i.igst_id / 100+((i.item_rt-i.discount)*sum(d.rc_qty)), ( ((i.item_rt - i.discount ) *sum(d.rc_qty) * i.cgst_id / 100)+((i.item_rt-i.discount)*sum(d.rc_qty)) ) + (( (i.item_rt - i.discount ) * sum(d.rc_qty) * i.sgst_id / 100 ))) AS total,sum(st.qty) as qty,concat(pd.prod_name,'(Part No.:',pd.part_no,', Article No.:',pd.article_no,', Model No.:',pd.model_no,', Make:',pd.prod_make,', Description:',pd.prod_desc,')') as prod_name, SUM(d.rc_qty) AS total_rc_qty, SUM(st.qty) AS total_qty"
    schema = '''td_po_basic b 
    left join td_po_items i on b.sl_no=i.po_sl_no 
    left join md_product pd on pd.sl_no=i.item_id
    join td_item_delivery_details d on d.po_no=b.po_no and d.prod_id=i.item_id 
