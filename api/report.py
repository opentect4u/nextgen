...
async def get_project_po(id: mrnprojreport):
    ...
    if id.type == 'P':  # Project type
        select = """
            ...
            pb.po_no, i.quantity,
            SUM(DISTINCT d.rc_qty) AS rc_qty,
...
async def getprojectpoc(id:mrnpur):
    select = f"@a:=@a+1 '#', b.po_no 'PO No.',DATE_FORMAT(b.po_issue_date,'%d/%m/%Y') 'Date',IF(b.po_status='A','Approved',IF(b.po_status='U','Approval Pending',IF(b.po_status='D','Delivered','Partial Delivery'))) 'Status',COALESCE(p.proj_name,'Warehouse') 'Intended For',v.vendor_name 'Vendor',b.created_by 'Created By', ROUND(SUM(i.quantity * (i.item_rt - i.discount) * ((COALESCE(cgst.gst_rate, 0) + COALESCE(sgst.gst_rate, 0) + COALESCE(igst.gst_rate, 0)) / 100)), 2) AS 'GST', ROUND(SUM(i.quantity * (i.item_rt - i.discount)), 2) 'Basic Value', ROUND(SUM(i.quantity * (i.item_rt - i.discount) + i.quantity * (i.item_rt - i.discount) * ((COALESCE(cgst.gst_rate, 0) + COALESCE(sgst.gst_rate, 0) + COALESCE(igst.gst_rate, 0))) / 100), 2) AS 'Gross Value'"
    schema = f"td_po_basic b left join td_project p ON p.sl_no=b.project_id join md_vendor v ON v.sl_no=b.vendor_id JOIN td_po_items i ON i.po_sl_no = b.sl_no  LEFT JOIN md_gst cgst ON cgst.sl_no = i.cgst_id LEFT JOIN md_gst sgst ON sgst.sl_no = i.sgst_id LEFT JOIN md_gst igst ON igst.sl_no = i.igst_id cross join (SELECT @a := 0) AS a group by b.po_no"
    where = f""
    order = ""
    flag = 1 
    result = await db_select(select, schema, where, order, flag)
