...
async def getMinReq(dt:GetStock):
    ...
    select = f"distinct req.sl_no,req.req_date,req.req_no,r.item_id,sum(r.approved_qty) as approved_qty,sum(r.cancelled_qty) as cancelled_qty,sum(st.qty) as stock_out_qty,(select sum(qty) where proj_id={dt.proj_id} and item_id={dt.prod_id} group by ref_no) as return_qty"
