...
async def item_dtls(data:ProjId):
    select = f"c.prod_name, c.sl_no prod_id,c.part_no,c.model_no,c.part_no, sum(b.rc_qty) tot_rc_qty,(SELECT SUM(qty*in_out_flag) FROM `td_stock_new` WHERE item_id=c.sl_no and proj_id={data.Proj_id}) as project_stock, (SELECT SUM(qty*in_out_flag) FROM `td_stock_new` WHERE item_id=c.sl_no and proj_id='0') as warehouse_stock"
...
@poRouter.post('/req_item_dtls')
async def req_item_dtl(data:ReqId):
    select = "a.sl_no, a.last_req_id, a.req_no, a.item_id, b.prod_name,b.part_no,b.model_no,b.article_no, a.rc_qty, a.req_qty, (SELECT SUM(qty*in_out_flag) FROM `td_stock_new` WHERE item_id=a.item_id and proj_id=c.project_id) as project_stock, (SELECT SUM(qty*in_out_flag) FROM `td_stock_new` WHERE item_id=a.item_id and proj_id='0') as warehouse_stock, c.project_id"
...
async def req_item_dtls(data:MrnId):
    select = "i.sl_no,i.po_sl_no,i.item_id,i.quantity,i.currency,p.prod_name,p.part_no,p.article_no,p.model_no,d.rc_qty,d.sl,d.sl_no as item_sl,d.remarks,d.invoice,t.invoice_dt,d.mrn_no,d.created_at,d.created_by"
