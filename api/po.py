...
async def getParentPoDate(po_no:GetInvList):
    ...
    where1 = f"b.po_no not in (select po_no from td_item_delivery_invoice) and b.po_no not in (select po_no from td_vendor_to_client where approve_flag='A')" if po_no.po_no=='' else f"b.po_no='{po_no.po_no}'"
    order1 = ""
    flag1 = 1
    result1 = await db_select(select1, schema1, where1, order1, flag1)
    print(result1, 'RESULT1')
