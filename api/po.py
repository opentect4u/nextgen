...
#     return result
    ...
    select = "d.invoice,d.invoice_dt,d.del_no,d.po_no,pr.sl_no, b.sl_no as del_sl,pr.proj_name,b.vendor_id,v.vendor_name,i.sl_no item_delivery_no,i.prod_id,p.prod_name,p.prod_make,p.part_no"
    schema = '''td_vendor_to_client d left join td_po_basic b on b.po_no = d.po_no left join td_project pr on pr.sl_no = b.project_id left join td_vtoc_items i on i.invoice=d.invoice left join md_product p on i.prod_id = p.sl_no left join md_vendor v on v.sl_no=b.vendor_id
...
async def getMinReq(id:GetMinReq):
    ...
    res_dt = {}
    select = "distinct r.sl_no,r.req_no,r.approve_flag,r.reason,r.intended_for,r.req_date,r.project_id,r.client_id,r.req_type,r.purpose,m.req_no as min_req_no,p.proj_name"
    table = "td_requisition r join td_min m on r.req_no=m.req_no join td_project p on r.project_id=p.sl_no"
    where = f""
    order = "ORDER BY r.req_date DESC"
    flag = 1
    result = await db_select(select, table, where, order, flag)
