...
'''
    where = f"(b.project_id='{id.project_id}' or b.vendor_id='{id.vendor_id}' or p.part_no like '%{id.part_no}%' or i.item_id='{id.prod_id}' or b.po_issue_date>={id.from_dt} and b.po_issue_date<={id.to_dt}) AND b.project_id IS NOT NULL AND b.vendor_id IS NOT NULL AND p.part_no IS NOT NULL and i.item_id is not null and b.issue_date is not null "
    order = "ORDER BY b.created_at DESC"
    flag = 1
    result = await db_select(select, schema, where, order, flag)
    # print(result, 'RESULT')
