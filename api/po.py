...
async def getcategory(id:GetRows):
    ...
    print(id.id)
    res_dt = {}
    select = "sl_no as `#`,po_no as `PO No.`,customer_article_no as `Article No./Part No.`,order_qty as `Order Quantity`,approved_qty as `Approved Quantity`, shipped_qty as `Shipped Quantity`, po_issue_dt as `PO Issue Date`, po_approve_dt as `Approval Date`,sie_sale_ord as `Siemens Sale Order`,customer_no as `Customer No.`, net_price as `Net Price`,total_price as `Total Price`,order_dt as `Order Date`"
