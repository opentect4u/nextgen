...
async def getprojectpoc(id:GetPoForTc):
    ...
    select = "i.sl_no,i.po_sl_no,i.item_id,i.quantity,i.item_rt,i.discount_percent,i.discount,p.prod_name,d.rc_qty,c.tc_qty"
    schema = f"td_po_items i left join md_product p on i.item_id=p.sl_no left join td_item_delivery_details d on i.item_id=d.prod_id and d.po_no='{id.po_no}' LEFT JOIN (SELECT po_no,item,SUM(tc_qty) AS tc_qty FROM td_test_cert WHERE po_no = '{id.po_no}' GROUP BY     po_no, item ) tc ON i.item_id = tc.item AND i.po_sl_no = '{id.id}'"
