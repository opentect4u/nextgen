...
async def getprojectpoc(id:GetPo):
    ...
    select =  "distinct @a:=@a+1 serial_number,b.po_no,'Siemens' as vend_ref ,b.sl_no as po_id,t.po_issue_dt as po_date,'P'as type ,'A' po_status,'Approved' po_status_val, IF(b.po_type='P','Project-Specific', IF(b.proj_id!=0, 'General','')) po_type,b.project_id,p.proj_name,b.sl_no,'Y' fresh_flag,'N','' amend_note, (select count(*) from td_item_delivery_invoice where po_no = b.po_no) as invoice_count,inv.approve_flag,inv.invoice,inv.mrn_no"
