...
async def save_trans(data:GetPurItemForPo):
    ...
    select1 = "i.pur_no,sum(i.qty) as qty,i.item_id,p.prod_name"
    schema1 = "td_purchase_items i,md_product p"
    where1 = f"i.item_id=p.sl_no and i.pur_no in ({pur_no}) group by iitem_id"
    order1 = ""
    flag1 =  1
    result1 = await db_select(select1, schema1, where1, order1, flag1)
...
async def save_trans(data:GetPurItem):
    ...
    select = "t.pur_no,t.pur_proj, GROUP_CONCAT(i.item_id) AS item_id,GROUP_CONCAT(p.prod_name) AS prod_name,tp.proj_name, GROUP_CONCAT(p.prod_make) AS prod_make,GROUP_CONCAT(p.part_no) AS part_no, GROUP_CONCAT(p.article_no) AS article_no,GROUP_CONCAT(p.model_no) AS model_no"
