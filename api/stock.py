...
async def save_trans(data:GetLog):
    ...
    if result3['msg'][0]['del_flag'] != '3':
        ...
        where =f"del_no in (select del_no from td_vtoc_items where po_no in  (select po_no from td_po_basic where pur_req like '%{data.pur_no.lstrip('PR-')}%' and approve_flag='A')) and item_id = {data.item_id}"
        order = ""
        flag =  1
        result = await db_select(select, schema, where, order, flag)
        print(result, 'RESULT')
