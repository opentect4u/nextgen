...
async def getstock(data:CheckItem):
    ...
    where = f"i.item_id='{data.item_id}' and i.approve_flag='P' and t.trans_no=i.trans_no"
...
async def save_trans(data:SaveTrans):
    ...
    values = f"'TWP-{tno}', '{data.trans_dt}', '{data.intended_for}', {data.client_id},{0}, '{data.project_id}','{data.purpose}','{data.user}','{formatted_dt}'"
...
async def save_trans(data:GetTrans):
    ...
    select = "t.trans_no,t.trans_dt,t.created_by,t.created_at,t.sl_no,p.proj_name,t.to_proj_id"
    schema = "td_transfer t,td_project p"
    where = f"sl_no='{data.id}' and trans_no like '%{'TWP-'}%' and p.sl_no=t.to_proj_id" if data.id>0 else f"trans_no like '%{'TWP-'}%'  and p.sl_no=t.to_proj_id"
