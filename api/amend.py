...
async def addpoamend(data:GetPo):
    ...
    fields= f'''SELECT NULL sl_no, po_id, "{am_po_no['msg']['am_po']}" po_no, po_date, po_type, po_issue_date, project_id, vendor_id, 'P' po_status, 'Y' fresh_flag, 0 active_step, po_no parent_po_no, 'Y' amend_flag, created_by, "{formatted_dt}" created_at, NULL modified_by, NULL modified_at FROM td_po_basic WHERE sl_no = {data.id}'''
    ...
    try:
        if(lastID > 0):
            ...
            fields3= f'SELECT NULL sl_no, "{lastID}" po_sl_no, item_id, quantity,item_rt, discount_percent, discount, unit_id, cgst_id, sgst_id, igst_id, delivery_dt,delivery_to,created_by,created_at, NULL modified_by, NULL modified_at FROM td_po_items WHERE po_sl_no = "{data.id}"'
