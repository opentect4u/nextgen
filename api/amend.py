...
async def addpoamend(data:GetPo):
    ...
    am_po_no = f'{parent_po['msg']['po_no']}-{str(am_po_row['msg']['po_no']).split('.')[0]}' if amend_flag['msg']['amend_flag']=='N' else f'{str(parent_po['msg']['po_no']).split('-')[0]}-{str(am_po_row['msg']['po_no']).split('.')[0]}'
