...
async def addpoamend(data:GetPo):
    ...
    fields= f'''SELECT NULL sl_no, po_id, "{am_po_no['msg']['am_po']}" po_no, po_date, po_type, po_issue_date, project_id, vendor_id, 'P' po_status, 'Y' fresh_flag, po_no parent_po_no, 'Y' amend_flag, created_by, created_at, NULL modified_by, NULL modified_at FROM td_po_basic WHERE sl_no = {data.id}'''
    ...
    try:
        if(lastID > 0):
            fields1= f'SELECT NULL sl_no, "{lastID}" po_sl_no,bill_to,ship_to,ware_house_flag,po_notes,created_by,created_at, NULL modified_by, NULL modified_at FROM td_po_delivery WHERE po_sl_no = "{data.id}"'
            ...
            fields3= f'SELECT NULL sl_no, "{lastID}" po_sl_no, item_id, quantity, item_rt, discount, unit_id, cgst_id, sgst_id, igst_id, delivery_dt,created_by,created_at, NULL modified_by, NULL modified_at FROM td_po_items WHERE po_sl_no = "{data.id}"'
            ...
            fields5= f'SELECT NULL sl_no, "{lastID}" po_sl_no, price_basis, price_basis_desc, packing_fwd_extra, packing_fwd_extra_val, packing_fwd_val, freight_ins, freight_ins_val, test_certificate, test_certificate_desc,ld_date, ld_date_desc, ld_val, ld_val_desc, ld_val_per, min_per, warranty_guarantee, duration, duration_value, o_m_manual, o_m_desc, operation_installation, operation_installation_desc, packing_type, manufacture_clearance, manufacture_clearance_desc,created_by,created_at, NULL modified_by, NULL modified_at FROM td_po_terms_condition WHERE po_sl_no = "{data.id}"'
