...
async def addpoamend(data:GetPo):
    ...
    try:
        if(lastID > 0):
            ...
            fields5= f'SELECT NULL sl_no, "{lastID}" po_sl_no, price_basis, price_basis_desc, packing_fwd_extra, packing_fwd_extra_val, packing_fwd_val, freight_ins, freight_ins_val,ins, ins_val, test_certificate, test_certificate_desc,ld_date, ld_date_desc, ld_val, ld_val_desc, ld_val_per, min_per, warranty_guarantee, dispatch_dt,comm_dt, duration, duration_value, o_m_manual, o_m_desc, operation_installation, operation_installation_desc, packing_type,packing_val, manufacture_clearance, manufacture_clearance_desc,created_by,created_at, NULL modified_by, NULL modified_at FROM td_po_terms_condition WHERE po_sl_no = "{data.id}"'
